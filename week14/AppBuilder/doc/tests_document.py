...
class DocumentViewsTest(TestCase):
    ...
    def test_doc_list_view(self):
        self.assertEqual(reverse('document_list'), '/doc/')
        Document.objects.create(**self.doc1)
    ...
    def test_doc_detail_view(self):
        Document.objects.create(**self.doc1)
        self.assertEqual(reverse('document_detail', args='1'), '/doc/1')
        self.assertEqual(reverse('document_detail', args='2'), '/doc/2')
    ...
    def test_doc_add_view(self):
        ...
        # Add without Login
        response = self.client.post(reverse('document_add'), self.doc1)
        response = self.client.post(reverse('document_add'), self.doc2)
        self.assertEqual(response.status_code, 302)
    ...
    def test_doc_edit_view(self):
        ...
        response = self.client.post(reverse('document_edit', args='1'), self.doc2)
        self.assertEqual(response.status_code, 302)
    ...
    def test_doc_delete_view(self):
        self.login()
        Document.objects.create(**self.doc1)
        self.assertEqual(reverse('document_delete', args='1'), '/doc/1/delete')
